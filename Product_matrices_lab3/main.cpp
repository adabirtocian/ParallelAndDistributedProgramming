...
int main()
{
	...
	if (METHOD == "threads")
	{
		...
		for (int i = 0; i < NO_TASKS; ++i)
		{
			if (i == NO_TASKS - 1)
			{
				threads.push_back(std::thread(multiplyMatrices, i * NO_ELEMS_PER_TASK, ROWS_RESULT * COLS_RESULT - 1, multiplication));
			}
			else {
				threads.push_back(std::thread(multiplyMatrices, i * NO_ELEMS_PER_TASK, (i + 1) * NO_ELEMS_PER_TASK - 1, multiplication));
			}
		}
		for (int i = 0; i < NO_TASKS; ++i)
		{
